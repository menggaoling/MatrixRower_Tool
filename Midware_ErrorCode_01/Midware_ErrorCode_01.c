...
#else  //跟 ERROR_TYPE 排列顺序要一致      错误代码,C等级最高, A最小
...
{   
  ...
  errorCode[TYPE_FAIL].status           = Midware_MachineType_Get_Type() ? 0 : 1;
    ...
    Midware_Temp_Get_Error(&TempError);                                           //获取Temp 状态
    ...
    errorCode[DIGTIAL_RX_ERROR].status = DigitalError.Digital_CRC_Error ? 1 : 0;
    errorCode[FALSH_WRITE].status      = NvFlashError.write ? 1 : 0;
    errorCode[FALSH_READ].status       = NvFlashError.read ? 1 : 0;
#endif   
  UCHAR by_Led = 0;
...
//priority,from 0 to 0xff,the highest priority on the top 
